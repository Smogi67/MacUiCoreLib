--[[
╔══════════════════════════════════════════════════════════════════════════════╗
║         LIQUID GLASS UI LIBRARY  ·  Core Module                             ║
║         macOS Sequoia-style settings window — fully configurable             ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  QUICK START                                                                 ║
║  ─────────────────────────────────────────────────────────────────────────  ║
║  This file is the LIBRARY core. It is meant to be loaded from a separate     ║
║  UserConfig script via:                                                      ║
║                                                                              ║
║      local LiquidGlass = loadstring(game:HttpGet("<raw url>"))()             ║
║                                                                              ║
║  Then call LiquidGlass:SetConfig{...} and LiquidGlass:AddTab(...) in the     ║
║  UserConfig script. Do not edit this file to add tabs — put tabs in your     ║
║  UserConfig instead.                                                         ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  EXAMPLE (paste into your UserConfig.lua, after the loadstring line)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
-- DI icon assets (image-based, replacing previous Unicode glyphs).
-- Loaded from GitHub via getcustomasset() on first run, then cached locally.
-- If getcustomasset is unavailable, icons won't render (graceful degradation).
local DI_TYPE_ICON = {
	toggle   = "",   -- "Toggle on"
	slider   = "",   -- "Slider"
	dropdown = "",   -- "Dropdown selected"
	button   = "",   -- "Toggle off" (used for buttons)
...
local function loadDIIcons()
	...
	local ICON_MAP = {
		toggle   = "Toggle on.png",
		slider   = "Slider.png",
		dropdown = "Dropdown selected.png",
		button   = "Toggle off.png",
	...
	for key, fileName in pairs(ICON_MAP) do
		...
		if exists then
			...
			local url = GITHUB_BASE .. "/" .. safeFileName
			local ok, data = pcall(function() return game:HttpGet(url) end)
			-- Guard against GitHub returning a 404 HTML/text page instead of binary
...
-- Icon (image-based; tinted white to match previous text-glyph look)
diIcon=Instance.new("ImageLabel"); diIcon.Size=UDim2.fromOffset(22,DI_H)
diIcon.Position=UDim2.fromOffset(16,0); diIcon.BackgroundTransparency=1
diIcon.Image=""; diIcon.ImageColor3=Color3.fromRGB(255,255,255)
diIcon.ScaleType=Enum.ScaleType.Fit
diIcon.ZIndex=102; diIcon.Parent=diContent
...
local function diExpand(ctrlType, labelText, valueText, isDragging)
	...
	diIcon.Image = ""; diLabel.Text = ""; diBadgeLbl.Text = ""
	diBadge.Visible = false
	diProgress.Visible = false
...
-- Smoothly transition the DI icon and accent color when the notification type
-- changes in-place (e.g. slider → warning while dragging above a threshold).
-- Crossfades the icon by tweening ImageTransparency out, swapping, then back in.
-- Tweens the accent bar and progress bar color simultaneously.
local function diUpgradeStyle(icon, accent)
	-- Color transition: short tween on accent bar and progress bar
	tween(diAccent,    0.18, {BackgroundColor3 = accent}, Enum.EasingStyle.Quad)
	tween(diProgress,  0.18, {BackgroundColor3 = accent}, Enum.EasingStyle.Quad)
	-- Icon crossfade: fade out, swap image, fade back in
	if diIcon.Image ~= icon then
		tween(diIcon, 0.08, {ImageTransparency = 1}, Enum.EasingStyle.Quad)
		task.delay(0.08, function()
			diIcon.Image = icon
			tween(diIcon, 0.12, {ImageTransparency = 0}, Enum.EasingStyle.Quad)
		end)
	end
end
		...
		if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
			...
			if callback then
				...
				if not diNotifyCalled and DI_SHOWING and diCurrentType ~= "slider" then
					...
					diUpgradeStyle(
						DI_TYPE_ICON["slider"] or DI_TYPE_ICON.custom,
						DI_TYPE_COLOR["slider"] or T.sliderFill
					)
				end
			end
		end
